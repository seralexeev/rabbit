...
import { findObjectKeys } from './object.ts';
...
export class JsonConverter {
    ...
    public constructor(options: { basePath: string }) {
    ...
    protected isFieldConvertible(key: Key): boolean {
        typeof key; // make typescript happy
        return true;
    }
    ...
    protected convertBuffer(buffer: ArrayBufferLike) {
        typeof buffer; // make typescript happy
        return '[object Buffer]';
    }

    protected convertProxy(proxy: object): unknown {
        typeof proxy; // make typescript happy
        return '[object Proxy]';
    }
    ...
    protected convertPromise(obj: Promise<unknown>): unknown {
        typeof obj; // make typescript happy
    ...
    protected convertStream(stream: { pipe?: unknown }): unknown {
        typeof stream; // make typescript happy
...
export const converter = new PragmaticJsonConverter({ basePath: typeof process === 'undefined' ? '/' : process.cwd() });
