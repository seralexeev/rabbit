...
export const values = <T extends object>(value: T): T[keyof T][] => {
    return Object.values(value);
};

export const entries = <T extends object>(value: T): [keyof T, T[keyof T]][] => {
    return Object.entries(value) as [keyof T, T[keyof T]][];
};

export const fromEntries = <TKey extends PropertyKey, TValue>(
    entries: Array<readonly [TKey, TValue]> | undefined | null,
): Record<TKey, TValue | undefined> => {
    return (entries ?? []).reduce(
        (acc, [key, value]) => {
            acc[key] = value;
            return acc;
        },
        {} as Record<TKey, TValue | undefined>,
    );
};
